...
4. 配置企业微信应用的成员可见范围和可信 IP。2022 年 6 月 20 日后创建的自建应用，需要通过固定可信 IP 转发时，在“微信消息转发代理地址”填写企业微信 API 反向代理根地址（不含 `/cgi-bin`，配置键为 `apiBaseUrl`）。不使用代理时保留默认值 `https://qyapi.weixin.qq.com`；新建、编辑已有空配置及清空后保存都会补回该默认值，后端也兼容空值并使用默认地址。通知发送、访问令牌获取、指令结果回复和菜单同步均使用此地址。该代理会接收应用 Secret 和访问令牌，只使用自己管理或信任的服务器；默认要求 HTTPS 并阻止内网地址，内网代理需显式开启“允许内网代理”。
