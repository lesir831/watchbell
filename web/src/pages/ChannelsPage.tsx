...
const channelSchemas: Record<ChannelType, { name: string; description: string; fields: PluginConfigField[]; defaults: Record<string, unknown> }> = {
  ...
  wecom: {
    ...
    fields: [
      ...
      { key: 'apiBaseUrl', label: '微信消息转发代理地址', type: 'url', description: '2022 年 6 月 20 日后创建的自建应用，需要通过固定可信 IP 转发时填写 HTTP 或 HTTPS 代理根地址（不含 /cgi-bin）。不使用代理时请保留默认值 https://qyapi.weixin.qq.com。代理会接收应用 Secret，请仅使用可信服务。' },
      { key: 'allowPrivate', label: '允许内网代理', type: 'boolean', description: '仅连接自己管理的内网/本机代理时开启。公网 HTTP 代理无需开启此项。' },
